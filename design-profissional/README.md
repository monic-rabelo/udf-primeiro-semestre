## Projetos Design profissional

### Atividade-GitHub

Atividade básica para testar e praticar os comandos básicos do GitHub
* Uso de git init, git add .
* Git commit -m e git pull/push
* HTML simples
  
---

### A1 

Subpasta com os materiais para a avaliação 1:
* Certificado de evento tech (networking)
* Currículo profissional em LaTeX feito no editor Overleaf
* PDF com vaga de estágio em informática
* Portfólio com um projeto relevante: projeto de struct da matéria de algoritmos.
